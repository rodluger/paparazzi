...
\begin{align}
    ...
    *
    \,
    \bvec{a}(\lnlam_0)
    %\mathbf{g}^\top(\lnlam_0) \, \mathbf{R}(t)
    %*
    %\bvec{a}(\lnlam_0)
    \quad ,
\end{align}
...
set of all points on the visible disk where \xxx{what???} 
...
%
\xxx{(this should be $\lnlam - \lnlam_0$, no?)}
%
\begin{align}
    x(\lnlam_0) &= 
...
\begin{figure}[p!]
    ...
    \oscaption{linalg}{%
        ...
        each measured at $K$ wavelength bins. The matrix on the right hand 
        side, $\bvec{D}$, is
        the Doppler design matrix, constructed from $M \times N$ Toeplitz
        matrices, each of shape $K \times (K + W)$, where $W$ is the width
        of the convolution kernel and $N$ is the number of spherical harmonic
        ...
        of the $M$ stellar phases observed. Finally, the vector at the 
        right is the weight vector $\bvec{a}$, a sequence of $N$ spectral 
        components, each of length $K + W$. In this example, each segment of
...
\begin{figure}[p!]
    ...
    \oscaption{linalg_u}{%
        ...
        of the model on the spherical harmonic coefficients $\bvec{u}$.
        \label{fig:linalg_u}
    }
    \end{centering}
\end{figure}
