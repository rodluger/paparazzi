...
\begin{figure}[h!]
    ...
    \oscaption{g}{%
        The Doppler $\g$ basis for a rigidly rotating star
...
\end{align}
%
Figure~\ref{fig:g} shows the $\g$ basis computed from
...
\end{align}
%
where we introduce the \emph{Doppler design matrix} $\bvec{D}$, constructed
by concatenating the Toeplitz matrices for each of the components of the
convolution kernel $\g^\top(\lnlam, \Dargs)$ after rotating it (via $\R(t)$) 
...
$\g$ functions (each of length $W$) to the correct stellar 
rotational phase, an operation that scales approximately
as $W \lmax^3$,
followed by $(\lmax + 1)^2$
linear convolutions of the rotated $\g$ functions with the 
