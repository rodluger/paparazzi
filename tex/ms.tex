...
uncertainties.
\xxx{%
Data-driven modeling at its finest $\cdot$ %
Works on blended lines $\cdot$ %
Uses the whole spectrum $\cdot$ %
Uncertainties FTW $\cdot$ %
Probably works on slow rotators $\cdot$ %
Differentiable! $\cdot$ %
Fast! $\cdot$ %
Use in MCMC to marginalize over inclination and stuff $\cdot$ %
}
%
...
%
\xxx{Check out \citet{Luger2019} and \citet{Bedell2019} and stuff.}
...
\begin{figure}[t!]
    \begin{centering}
    \includegraphics[width=0.6\linewidth]{figures/ld.pdf}
    \oscaption{ld}{%
...
We generated our spectrum at high resolution 
...
our spectrum consists of $K = 201$ log-wavelength bins in the range
$-3.3\times 10^{-4} < \lnlam < 3.3\times 10^{-4}$ plus an additional
%
\begin{align}
    W - 1 &= \frac{40~\mathrm{km/s}}{c} \frac{201}{3.3 \times 10^{-4}} - 1 \nonumber \\
          &= 80
\end{align}
%
log-wavelength bins of padding (40 on either side) to eliminate edge
effects due to the convolution. As in \citet{Vogt1987},
we assume a projected velocity of $v\sin i = 40$ km/s and an inclination
$i = 40^\circ$.



% Our default SNR is about 75.



Recover images with an effective resolution of $12^\circ$.
...
\begin{figure}[t!]
    ...
    \oscaption{spot_setup}{%
        ...
        Gaussian absorption lines (top right). The specific intensity
        ...
        %
        To generate the synthetic dataset, the star is inclined at
        ...
        $-180^\circ$ and $180^\circ$ (lower panel).
        %
...
\begin{figure}[p!]
    ...
    \oscaption{spot_y1bs}{%
        ...
        (dotted orange line), and the inferred spectrum (orange line, with
        shaded $1\sigma$ uncertainty). The grey regions indicate wavelength
...
\begin{figure}[t!]
    ...
    \oscaption{twospec}{%
        ...
        ``eigenspectra'' ($s_1$ and $s_2$).
        As in Figure~\ref{fig:spot_y1bs}, we assume
        we do not know the map, the baseline, or either of the eigenspectra.
...
\xxx{We are not addressing differential rotation and convective blueshift.}
\xxx{Telluric lines could be treated much as in Figure~\ref{fig:twospec}.}
\xxx{Instrumental profile is another Toeplitz matrix.}
