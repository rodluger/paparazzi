...
Gaussian prior on $\alm$ with mean $\boldsymbol{\mu}$ and 
covariance $\boldsymbol{\Lambda}$, the maximum \emph{a posteriori} (MAP) 
solution for $\alm$ is
%
\begin{align}
    \bvec{\hat{a}} &= 
...
the map of the star ($\bvec{\hat{a}}$) and its uncertainty
($\boldsymbol{\Sigma}_\mathbf{\hat{a}}$) in under a second for
a typical dataset.

There is, however, a catch: for most practical purposes, it is 
difficult to find a good Gaussian prior for $\alm$. Usually
...
on $\alm$ is \emph{not}. To understand this, consider the
matrix $\bvec{A}$, constructed by reshaping $\alm$ into
a matrix of shape $(N \times K')$, whose rows correspond
to the spectral components associated with each spherical harmonic coefficient.
By performing singular value decomposition (SVD) on this matrix, we may
write
%
\begin{align}
    \bvec{A} = \bvec{U} \, \bvec{S} \, \bvec{V}^\top
\end{align}
%
where $\bvec{U}$ is a matrix of shape $(N \times P)$, $\bvec{V}^\top$ is a 
matrix of shape $(P \times K')$, and $\bvec{S}$ is a diagonal matrix of 
shape $(P \times P)$ for some integer $P$.
...
\section{The VOGTSTAR}
\label{sec:vogtstar}

\begin{figure}[p!]
    \begin{centering}
    \includegraphics[width=0.49\linewidth]{figures/vogtstar_u_timeseries.pdf}
    \includegraphics[width=0.49\linewidth]{figures/vogtstar_u_rect.pdf}
    \oscaption{vogtstar_u}{%
        Solution to the \vogtstar problem at high SNR when both 
        the rest frame spectrum and the baseline are known.
        \label{fig:vogtstar_u}
    }
    \end{centering}
\end{figure}
...
\begin{figure}[p!]
    \begin{centering}
    \includegraphics[width=0.49\linewidth]{figures/vogtstar_ub_timeseries.pdf}
    \includegraphics[width=0.49\linewidth]{figures/vogtstar_ub_rect.pdf}
    \\[0.5em]
    \includegraphics[width=0.405\linewidth]{figures/vogtstar_ub_baseline.pdf}
    \includegraphics[width=0.575\linewidth]{figures/vogtstar_ub_coeffs.pdf}
    \oscaption{vogtstar_ub}{%
        Similar to Figure~\ref{fig:vogtstar_u}, but assuming the baseline
        is not known.
        \label{fig:vogtstar_ub}
    }
    \end{centering}
\end{figure}
...
\begin{figure}[p!]
    \begin{centering}
    \includegraphics[width=0.49\linewidth]{figures/vogtstar_ubvT_timeseries.pdf}
    \includegraphics[width=0.49\linewidth]{figures/vogtstar_ubvT_rect.pdf}
    \\[0.5em]
    \includegraphics[width=0.405\linewidth]{figures/vogtstar_ubvT_baseline.pdf}
    \includegraphics[width=0.575\linewidth]{figures/vogtstar_ubvT_coeffs.pdf}
    \\[0.5em]
    \includegraphics[width=0.98\linewidth]{figures/vogtstar_ubvT_spectrum.pdf}
    \oscaption{vogtstar_ubvT}{%
        Similar to Figure~\ref{fig:vogtstar_u}, but assuming we have no
        information about either the spectrum, the baseline, or the map
        coefficients.
        \label{fig:vogtstar_ubvT}
    }
    \end{centering}
\end{figure}
...
\begin{figure}[h!]
    ...
    \oscaption{inclinations}{%
        Recovered \vogtstar map as a function of inclination.
