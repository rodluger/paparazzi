...
\begin{proof}{faa_di_bruno}
    ...
    \sum_{k=1}^n P_{nk} \frac{\mathrm{d}^k I_0(\lambda_0)}{\mathrm{d}\lambda_0^k} \lambda_0^k
\end{proof}
%
where
%
...
\begin{figure}[p!]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/convergence.pdf}
    \oscaption{convergence}{%
        Fractional root mean square error on the Taylor expansion of a
        Doppler-shifted Gaussian absorption line profile as a function of 
        the Doppler parameter 
        $\xi \equiv \beta / \left( \nicefrac{\sigma}{\mu}\right)$ 
        and the order of the expansion $N$. The Taylor series
        does not converge above $\xi \approx 3$.
...
\begin{figure}[p!]
    \centering
    \subfigure[Local absorption]{\label{fig:convergence_line:a}%
        \includegraphics[width=2.5in]{figures/local_spectrum.pdf}}
    \hspace{0.25in}
    \subfigure[Disk-integrated absorption]{\label{fig:convergence_line:b}%
        \includegraphics[width=2.5in]{figures/disk_integrated_spectrum.pdf}}
    \oscaption{convergence}{%
        Taylor expansion of a Doppler-shifted Gaussian absorption line
        (orange) for different values of the Doppler parameter 
        $\xi \equiv \beta / \left( \nicefrac{\sigma}{\mu}\right)$. 
        The dashed black curve is the line in the rest frame, and the blue
        curve is the shifted line computed numerically at high precision.
        \label{fig:convergence_line}
    }
\end{figure}
