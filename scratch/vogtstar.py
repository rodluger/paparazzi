...
if (clobber) or (not os.path.exists("%s.npz" % filename)):
    ...
    def get_vT(u):
        ...
        U = diags([np.ones(Kp) * u[n] for n in range(N)], offsets, shape=(N * Kp, Kp))
    ...
    # We assume we roughly know the spectrum...
    #vT_guess = I0_padded
    ...
    ax[0].plot(lam_padded, I0_padded)
    ax[0].plot(lam_padded, vT_guess)
    ...
    with pm.Model() as model:
        ...
        mu_u = np.zeros(N)
        mu_u[0] = 1.0
        cov_u = sig_u ** 2 * np.eye(N)
        cov_u[0, 0] = sig_u0 ** 2

        u = pm.MvNormal("u", mu_u, cov_u, shape=(N,), testval=u_guess)
        u = tt.reshape(u, (-1, 1))

        # The spectral basis
        mu_vT = np.ones(solver.Kp)
        cov_vT = sig_vT ** 2 * np.eye(solver.Kp)
        vT = pm.MvNormal("vT", mu_vT, cov_vT, shape=(solver.Kp,), 
                         testval=vT_guess)
    ...
    with model: 
        map_soln, info = xo.optimize(return_info=True)
        info = dict(info)
        info.pop("x", None)
    ...
    map_map.inc = inc
    map_map[1:, :] = map_soln["u"][1:] / map_soln["u"][0]
...
ax.plot(lam_padded, vT_guess.reshape(-1), "C1-", alpha=0.5, lw=0.5, label="Guess")
