...
\begin{algorithm}
    ...
    \end{spacing}
    \label{alg:solve1}
    \caption{Solution to the L2 linear least squares problem for the spherical harmonic representation $\bvec{y}$ of the stellar surface map. Given an unnormalized spectral timeseries $\bvec{f}$, its covariance matrix $\boldsymbol{\Sigma}_\mathbf{f}$, a Gaussian prior on the map (mean $\boldsymbol{\mu}_\bvec{y}$, covariance $\boldsymbol{\Lambda}_\bvec{y}$), and the rest frame spectrum $\bvec{s}$, this function returns the maximum a posteriori (MAP) solution $\bvec{\hat{y}}$ to the surface map as well as its covariance $\boldsymbol{\Sigma}_\mathbf{\hat{y}}$. The $\mathrm{blockDiag}$ operation on the second line is given by Equation~(\ref{eq:inverse:S}).}
\end{algorithm}
        ...
        \begin{fminipage}{0.98\textwidth}
            ...
            \begin{algorithmic}
                ...
                \WHILE{$T > 1$}
                    ...
                    \boldsymbol{\Sigma}_\mathbf{f} 
                    \gets 
                    e^{\ln T} \mathrm{diag}(\boldsymbol{\sigma}_\mathbf{f} ) + \sigma_b^2
                    $
                    \STATE 
                    $
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \caption{%
            Similar to Figure~\ref{fig:spot_infer_y}, but assuming the baseline is not known.
        }
