...
The mean standard deviation in each wavelength bin across all 16 epochs in the noiseless data vector is about $2\times 10^{-2}$, so our choice of $\sigma_\bvec{f}$ corresponds to SNR $\sim$ 100 in each of the 200 wavelength bins of $\bvec{f}$.
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \caption{%
            ...
            The optimization step in this experiment ran in under 1 second.
        }
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \caption{%
            ...
            The optimization step in this experiment ran in about 5 seconds.
        }
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \caption{%
            ...
            The optimization step in this experiment ran in about 30 seconds.
        }
    ...
    \begin{centering}
        ...
        \caption{%
            Same as Figure~\ref{fig:spot_infer_ybs}, but for $10\times$ lower SNR.
             ...
             Once again, the optimization step in this experiment ran in about 30 seconds.
        }
...
%
After some tuning, we find it necessary to increase our prior variances slightly to get good fits to the data: we set $\boldsymbol{\Lambda}_\bvec{y} = 2.5\times 10^{-4} \bvec{I}$ and $\boldsymbol{\Lambda}_\bvec{s} = 5\times 10^{-3}\bvec{I}$.
%
...
%
We generate the synthetic dataset in the same way as in \S\ref{sec:spot_y1}: 16 spectra, each with 200 wavelength bins observed with SNR$\sim$100.
...
%
We also solve for a single scale parameter $r \sim \mathrm{Uniform}(0, 1)$, equal to the ratio of the continuum levels of the two spectra (whose true value is 0.5).
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \caption{%
            Recovered \texttt{STAR SPOT} map as a function of stellar inclination. 
...
Figure~\ref{fig:inclinations} shows the inferred map for the same problem setup as before, but this time varying the inclination between $10^\circ$ (nearly face-on) and $90^\circ$ (perfectly edge-on). We also use a different surface map, this time with the word \texttt{STAR} in the northern hemisphere and the word \texttt{SPOT} in the southern hemisphere, to assess our ability to infer information across the entire stellar surface. \xxx{Given the added complexity of the map, we increase the spherical harmonic degree to} $l_\mathrm{max} = 20$, increase the SNR of our data to 200, and set the covariance of the spatial prior to $\boldsymbol{\Lambda}_\bvec{y} = 3\times 10^{-5}\bvec{I}$.
As before, the inclination is assumed to be known exactly. 
\xxx{We further keep the true equatorial velocity of the star constant} across all panels, so that the figure corresponds to the same star viewed at different orientations. 
