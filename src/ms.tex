...
In the limit of infinite SNR, spectral resolution, and temporal sampling, and if the rest frame spectrum is known exactly, in many cases there \emph{are} unique solutions to the visible portion of the stellar surface\footnote{Recall that, except in the pathological case where $i = 90^\circ$, there is always a region on the ``back'' side of the star that is never in view at any phase. No amount of statistical or algorithmic cleverness can ever tell us what that region looks like!}.
...
Doppler imaging studies are notorious at producing features that are artificially elongated in latitude, just like the dark spot seen in the figure \xxx{(cite me)}.
...
In some restricted cases, it is possible to obtain the full surface map posterior analytically (\S\ref{sec:inverse}), but only when all priors are Gaussian and the inclination, limb darkening profile, and template spectrum are known exactly. In all other cases, one must compute an approximation to the posterior distribution over all these quantities numerically, such as with MCMC.

\xxx{
    Our model is differentiable, so we can easily use HMC. 
    Within HMC, we can learn hyperpriors and marginalize over the unknowns.
    Stress that it is NOT correct to infer things like active latitudes, differential rotation, etc. from point estimates obtained with Doppler imaging. Those will always be biased. You have to infer that stuff probabilistically!
}
