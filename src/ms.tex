...
Since those pioneering studies several decades ago, Doppler maps have now been obtained for a variety of targets, ranging from red giant stars \citep{Roettenbacher2017} to massive main sequence stars \xxx{(cite me)} to cool dwarfs \citep{Rosen2015}. Recently, \citet{Crossfield2014} produced a Doppler map of Luhman 16B, a brown dwarf in the solar neighborhood. Future observations with extremely large telescopes may even allow one to obtain Doppler maps of directly imaged extrasolar planets \citep{Snellen2014}.

As more and more stars are imaged in this way, it becomes tempting to interpret Doppler images in the broader context of stellar physics and evolution.
...
However, the vast majority of Doppler imaging studies produce \emph{point estimates}: that is, they usually report the maximum likelihood solution to the stellar surface map, with no formal assessment of its uncertainty.
...
%
First, Doppler imaging is usually an ill-posed problem, meaning there is no unique solution to the surface map given a spectroscopic dataset.
This issue is typically well understood \xxx{(cite me)}, and most studies circumvent this issue by employing restrictive priors to ensure the uniqueness of the solution. 
...
Moreover, changes in the local spectrum over the surface of the star (such as different line depths or the presence of different absorption lines within spots) can similarly bias the inferred map if these are not properly modeled \xxx{(cite me)}.
...
%
Fourth, and finally, most Doppler imaging codes are not open source \xxx{(cite me)}, nor are the data used to produce many of the maps found in the literature, making it difficult to reproduce results and compare Doppler images of different stars on the same footing.
...
Finally, we implement our algorithm within the open source \starry code package with ample documentation, examples, and tutorials.\footnote{\url{https://github.com/rodluger/starry}}.
...
Typically, cross-validation consists of choosing a specific value for the prior, masking one (``leave-one-out-cross-validation'') or several (``leave-p-out-cross-validation'') data points, optimizing the parameters of the model given this smaller dataset, then using the optimized model to predict the values of the missing data \xxx{(cite me)}.
...
\begin{figure}[t!]
    \begin{centering}
        ...
        \caption{%
            ...
            The rest frame spectrum is the same as in Figure~\ref{fig:spot_setup} and the star is inclined at $i = 40^\circ$ with equatorial velocity $v = 50{,}000$ km/s.
...
Most Doppler imaging studies rely on techniques like template matching \xxx{(cite me)} or least squares deconvolution \citep[LSD;][]{Donati1997} to infer a mean spectral line shape at each epoch, which is then treated as the input to the Doppler imaging step.
...
While Doppler imaging has classically been used to infer the surfaces of massive stars \xxx{(cite me)}, the study of \citet{Crossfield2014} used the method to infer the first Doppler map of a brown dwarf, which we reproduced in this work.
